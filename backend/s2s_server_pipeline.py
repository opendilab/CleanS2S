    ...
    def process(self, inputs: Dict[str, Union[str, int]]) -> Generator[Dict[str, Union[str, int, bool]], None, None]:
        ...
        Arguments:
            - inputs (Dict[str, Union[str, int]]): The input data acquired from queue_in. The data contains the \
                str format audio transcripted data and integer user input count.
        Returns (Yield):
            - output (Dict[str, Union[str, int, bool]]): The output data containing the transcripted question text, \
                the generated answer text, end flag for the current LLM generation, and the user input count.
        """
        ...
        prompt, user_input_count = inputs["data"], inputs["user_input_count"]
        count = 0
...
class LanguageModelAPIHandler(BaseHandler):
    ...
    def process(self, inputs: Dict[str, Union[str, int]]) -> Generator[Dict[str, Union[str, int, bool]], None, None]:
        ...
        Returns (Yield):
            - output (Dict[str, Union[str, int, bool]]): The output data containing the transcripted question text, \
                the generated answer text, end flag for the current LLM API generation, and the user input count.
        """
        # avoid concurrent inference, loop when working_event is set
        while self.working_event.is_set():
            time.sleep(0.1)
...
class CosyVoiceTTSHandler(BaseHandler):
    ...
    def process(self,
        ...
        llm_sentence = llm_output['answer_text']
        ...
        while True:
            ...
            if audio_chunk is self.stop_signal:
                break
            end_flag = llm_output['end_flag'] and i == total_cnt - 1
            if i == 0:
                ...
                yield {
                    ...
                    "end_flag": end_flag,
                    "user_input_count": llm_output['user_input_count']
                }
            else:
                yield {
                    'question_text': None,
